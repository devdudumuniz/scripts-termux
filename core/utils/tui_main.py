...
# -*- coding: utf-8 -*-
"""
DM Termux Pentest - Interface TUI Principal
Interface de usuário em modo texto com Rich
"""
...
/_/    \___/_/ /_/\__/\___/____/\__/  /_/   /_/   \__,_/_/ /_/ /_/\___/ 
        ...
        banner_panel = Panel(
            ...
            title="[bold]DM Pentest Framework v1.0.0[/bold]",
    ...
    def show_main_menu(self):
        ...
        table.add_column("Módulo", style="green", width=20)
        table.add_column("Descrição", style="white", width=50)
        
        # Módulos de plugins
        menu_items = [
            ("1", "OSINT", "Coleta de informações públicas"),
            ("2", "WEB", "Testes de aplicações web"),
            ("3", "REDE", "Análise e scanning de rede"),
            ("4", "EXPLOIT", "Ferramentas de exploração"),
            ("5", "FORENSIC", "Análise forense digital"),
            ("6", "AUTOMATION", "Automação de tarefas"),
            ("", "", ""),
            ("7", "ROOT-TOOLS", "Ferramentas que requerem ROOT"),
            ("8", "NO-ROOT-TOOLS", "Ferramentas sem ROOT"),
            ("", "", ""),
            ("9", "LOGS & EXPORT", "Visualizar logs e exportações"),
            ("10", "CONFIGURAÇÕES", "Ajustes do framework"),
            ("", "", ""),
    ...
    def run_module_menu(self, module_name):
        """Executa menu de um módulo específico"""
        self.console.clear()
        self.show_banner()
        ...
        if not module:
            self.console.print(f"[red]Módulo {module_name} não encontrado![/red]")
        ...
        if metadata:
            panel = Panel(
                f"[bold]{metadata.get('description', 'Sem descrição')}[/bold]",
        ...
        try:
            ...
            self.console.print(f"[red]Erro ao executar módulo: {e}[/red]")
            self.logger.error(f"Erro no módulo {module_name}: {e}")
    ...
    def show_exports(self):
        ...
        if not exports:
            self.console.print("[yellow]Nenhuma exportação encontrada[/yellow]")
        else:
    ...
    def show_settings(self):
        """Exibe e permite editar configurações"""
        self.console.clear()
        self.show_banner()
        
        self.console.print(Panel("[bold]Configurações[/bold]", border_style="cyan"))
        self.console.print()
    ...
    def run(self):
        ...
        while True:
            ...
            choice = Prompt.ask("[bold cyan]Escolha uma opção[/bold cyan]", default="0")
...
def main():
    """Função principal"""
    parser = argparse.ArgumentParser(description='DM Termux Pentest Framework')
    parser.add_argument('--mode', choices=['root', 'no-root', 'hybrid'], 
                       default='hybrid', help='Modo de operação')
